...
class _Holder:
    """A wrapper around a tensor to keep track of references to the underlying memory.

    Parameters
    ----------
    ms_tensor : ms_tensor
        The tensor that will be converted to a DLPack tensor and must be managed.
    """

    def __init__(self, ms_tensor: ms.Tensor) -> None:
        self.tensor = ms_tensor
        self.np_array = ms_tensor.asnumpy()
        self.data = self.np_array.ctypes.data_as(ctypes.c_void_p)
        self.shape = self.np_array.ctypes.shape_as(ctypes.c_int64)
        self.strides = self.np_array.ctypes.strides_as(ctypes.c_int64)
        for i in range(self.np_array.ndim):
            self.strides[i] //= self.np_array.itemsize
...
def to_dlpack(ms_tensor: ms.Tensor):
    """Convert a tensor to another type of dlpack compatible array.

    Parameters
    ----------
    ms_tensor : ms.Tensor
        The source tensor that will be converted.
    ...
    dl_managed_tensor.dl_tensor.ndim = holder.np_array.ndim
    dl_managed_tensor.dl_tensor.dtype = DLDataType.TYPE_MAP[str(ms_tensor.dtype)]
